    ...
    suspend fun generateMasterJson(projectId: Int, projectTitle: String): String {
        ...
        for (snippet in snippets) {
            ...
            for (cmd in latexCommands) {
                safeJson = safeJson.replace(cmd, "\\\\" + cmd.substring(1)) // Ensure it doesn't double escape if already escaped
            }
            // Better yet, just use a raw string literal or regex to escape backslashes not followed by valid JSON escapes
            // But we'll leave the simple replace for now.
