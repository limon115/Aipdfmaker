...
# We know we are already inside the workflows folder based on your 'ls' command
file_path = "build.yml"
...
end_str = 'export NDK=${ANDROID_NDK_HOME:-${ANDROID_NDK_LATEST_HOME:-}}'
...
if start_idx != -1 and end_idx != -1:
    ...
    # Slice out the old broken code and insert the new invincible block
    content = content[:start_idx] + new_block + content[end_idx:]
    
    # Force GitHub to pull down Git LFS files correctly during checkout
    if "lfs: true" not in content:
        content = content.replace("uses: actions/checkout@v4\n", "uses: actions/checkout@v4\n        with:\n          lfs: true\n")

    with open(file_path, "w") as f:
        f.write(content)
    print("✅ SUCCESS: Patched build.yml with invincible dynamic tarball downloading!")
else:
